    ...
    def forward(self, x, x_hallucination, encoder_padding_mask, encoder_padding_mask_hallucination, batch_len, lay_idx, attn_mask: Optional[Tensor] = None, kl_divergence=None, src_img_features=None):
        ...
        if src_img_features is not None:
            if lay_idx >= 4:
                ...
                x = x + src_img_features.transpose(0, 1)
                visual_hallucination_representation = src_img_features

        else:
            if lay_idx >= 4:

                src_img_features, gate = self.gating(x, visual_hallucination_representation)
                x = x + src_img_features.transpose(0,1)
...
class TransformerEncoderLayer_hallucination(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, hallucination, lay_idx , encoder_padding_mask,
        ...
        # hallucination = torch.fft.fft(hallucination, dim=-1)
        # hallucination = torch.fft.fft(hallucination.float(), dim=1)
        # hallucination = torch.real(hallucination).half()
...
class GatingMechanism(nn.Module):
    ...
    def forward(self, x, grid_img_features):
        ...
        img_features = torch.mul(gate, x.transpose(0,1))
