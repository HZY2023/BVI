...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        if src_img_features is not None:
            ...
            ########################对于Fashion-MMT数据集(每条文本包含任意数量的图像)需要进行下列操作########################################

            # # 用于提取数字的正则表达式
            # pattern = re.compile(r'/all-images-listdir/(\d+)/')
            # final_output = []
            #
            # # 使用字典来记录数字的出现次数
            # counts = {}
...
class GaussianKernel(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # # 扩展维度以便可以进行张量广播
        # for x in x1:
        #     x1_reshaped_1 = x.view(self.num_regions, 1, self.num_features)
        #     x1_reshaped.append(x1_reshaped_1)
        # for y in x2:
        #     x2_reshaped_2 = y.view(1, self.num_regions, self.num_features)
        #     x2_reshaped.append(x2_reshaped_2)
        # x1_reshaped = torch.cat(x1_reshaped, dim=1).transpose(0,1)
        # x2_reshaped = torch.cat(x2_reshaped, dim=0)

        # x1_reshaped = x1.unsqueeze(2)
...
class CustomModel(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        final_representation = torch.cat(representations, dim=0).to(device)  # 沿着特征维度连接
...
#             feature_1 = torch.tensor(self.folder_pairs[index][0])
#             feature_2 = torch.tensor(self.folder_pairs[index][1])
#         else:
#             feature_1 = torch.tensor(self.folder_pairs[0])
#             feature_2 = torch.tensor(self.folder_pairs[1])
#         return feature_1.cuda(), feature_2.cuda()


def print_cuda_memory():
    device = torch.device('cuda:0' if torch.cuda.is_available() else "cpu")
    allocated = torch.cuda.memory_allocated(device) / 1e6  # MBs
    reserved = torch.cuda.memory_reserved(device) / 1e6  # MBs
